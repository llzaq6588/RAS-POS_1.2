...
if ($code < 101) {
	...
	*/
	...
	$yearmonth = date("Y-m"); //date와 time이 변수로 선언된다. sold_book.php의 검색시스템을 위해 년도, 월, 시간은 분리되었다.
	$sellday = date("Y-m-d");
	$time = date("H:i:s");
	...
	{ //sell의 총 합은 totalprice라는 변수로 선언된다.
		...
		$sql2="INSERT INTO sold_book (yearmonth, sellday, time, totalprice) " .
				"VALUES ('$yearmonth', '$sellday', '$time', '$totalprice')"; //변수로 선언된 날짜, 시간, totalprice는 sold_book 매상장부 테이블로 전송된다.
